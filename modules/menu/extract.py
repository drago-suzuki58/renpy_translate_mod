    ...
    def extract_choices(lines, indent, start_line):
        ...
        while lines:
            ...
            if line_indent == indent + 2 and line.startswith('"'):
                ...
                choices.append(
                    [filename, start_line, choice, "Untranslated"]
                )  # 選択肢、行数、ファイル名をタプルとして追加
            elif line.startswith("menu:"):
                choices.extend(
                    extract_choices(
                        lines, line_indent + 2, start_line + len(choices) + 1
                    )
                )  # 行数を更新
            start_line += 1  # 行数を更新
        logs.logs(
            "DEBUG", "Extract_menu", f"Extracted {len(choices)} choices from {filename}"
        )
        return choices

    lines = text.split("\n")
    indent = (
        len(lines[0]) - len(lines[0].lstrip()) if lines[0].startswith("menu:") else 0
    )
