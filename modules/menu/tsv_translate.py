...
def main(
    ...
    logs.logs(
        "DEBUG", "Translate_menu", f"{input}, {output}, {fromlang}, {tolang}, {target}"
    )
    ...
    with open(input, "r", encoding="utf-8") as f:
        with open(output, "w", encoding="utf-8") as output_f:
            ...
            logs.logs(
                "INFO", "Translate_dialogue", f"Read {len(lines)} lines from {input}"
            )
            ...
            for line in lines:
                if line == "filename\tlinenumber\tchoice\ttranslated\n":
                    ...
                    logs.logs(
                        "ERROR", "Translate_dialogue", "This is not a menu TSV file."
                    )
                    return
                ...
                if choice in tlist:
                    logs.logs(
                        "DEBUG",
                        "Translate_dialogue",
                        f"Skip_3:\t{filename}, {linenumber}, {choice}, {translated_text}",
                    )
                    progress += 1
                    continue
                ...
                if target == [""]:
                    logs.logs(
                        "DEBUG",
                        "Translate_dialogue",
                        f"Pass:\t{filename}, {linenumber}, {choice}, {translated_text}",
                    )
                    pass
                    ...
                    logs.logs(
                        "DEBUG",
                        "Translate_dialogue",
                        f"Skip_1:\t{filename}, {linenumber}, {choice}, {translated_text}",
                    )
                    progress += 1
                    continue
                ...
                if progress < start_line:
                    logs.logs(
                        "DEBUG",
                        "Translate_dialogue",
                        f"Skip_2:\t{filename}, {linenumber}, {choice}, {translated_text}",
                    )
                    progress += 1
                    continue
                ...
                if choice:
                    ...
                    choice = base16.encode_matches(r"\[.*?\]", choice)
                    choice = base16.encode_matches(
                        r"\\.", choice
                    )  # \{encoded_text} の形式だと変換されてしまうと思うので|encoded_text|のように変換

                    for _ in range(5):  # 翻訳のタイムアウト時に最大5回再試行
                        try:
                            total_chars += len(choice)
                            translated_text = g_translator.translate(
                                choice, src=fromlang, dest=tolang
                            ).text
                            logs.logs(
                                "INFO",
                                "Translate_dialogue",
                                f"Translated:\t{original_choice} -> {translated_text}",
                            )
                            break
                            ...
                            logs.logs(
                                "WARNING",
                                "Translate_dialogue",
                                f"Google Translate API timed out. Retrying...:\t{_}/5 time:\tLine: {progress}",
                            )
                            continue
                    else:
                        logs.logs(
                            "ERROR",
                            "Translate_dialogue",
                            f"Google Translate API timed out 5 times. Skipping this line.\nLine: {progress}",
                        )
                        continue
                    ...
                    output_f.write(
                        f"{filename}\t{linenumber}\t{original_choice}\t{translated_text}\n"
                    )
                    logs.logs(
                        "DEBUG",
                        "Translate_dialogue",
                        f"Append:\t{filename}, {linenumber}, {original_choice}, {translated_text}",
                    )

                    logs.logs(
                        "INFO",
                        "Translate_dialogue",
                        f"Line {progress}/{total_lines}:\t{original_choice},\t{translated_text}",
                    )
                    progress += 1
