...
def main(
    ...
    logs.logs(
        "DEBUG",
        "Translate_dialogue",
        f"{input}, {output}, {fromlang}, {tolang}, {target}",
    )
            ...
            for line in lines:
                if line == "filename\tlinenumber\tidentifier\tcontents\n":
                    ...
                    logs.logs(
                        "ERROR",
                        "Translate_dialogue",
                        "This is not a dialogue TSV file.",
                    )
                    return
                ...
                if target == [""]:
                    logs.logs(
                        "DEBUG",
                        "Translate_dialogue",
                        f"Pass:\t{filename}, {linenumber}, {identifier}, {contents}",
                    )
                    pass
                    ...
                    logs.logs(
                        "DEBUG",
                        "Translate_dialogue",
                        f"Skip_1:\t{filename}, {linenumber}, {identifier}, {contents}",
                    )
                    progress += 1
                    continue
                ...
                if progress < start_line:
                    logs.logs(
                        "DEBUG",
                        "Translate_dialogue",
                        f"Skip_2:\t{filename}, {linenumber}, {identifier}, {contents}",
                    )
                    progress += 1
                    continue
                ...
                match_str = re.match(
                    r'^([^"]*)"((?:[^"\\]|\\.)*)"', contents
                )  # エスケープされていないダブルクォーテーションのみを対象にする(つまりセリフと人物を分ける)

                if match_str == None:
                    output_f.write(
                        f"{filename}\t{linenumber}\t{identifier}\t{contents}\n"
                    )

                    logs.logs(
                        "DEBUG",
                        "Translate_dialogue",
                        f"Append:\t{filename}, {linenumber}, {identifier}, {contents}",
                    )
                    logs.logs(
                        "INFO",
                        "Translate_dialogue",
                        f"Line {progress}/{total_lines}:\t{contents}",
                    )
                    ...
                    logs.logs(
                        "DEBUG",
                        "Translate_dialogue",
                        f"Skip_3:\t{filename}, {linenumber}, {identifier}, {contents}",
                    )
                    progress += 1
                    continue
                    ...
                    for _ in range(5):  # 翻訳のタイムアウト時に最大5回再試行
                        try:
                            ...
                            translated_text = g_translator.translate(
                                dialogue, src=fromlang, dest=tolang
                            ).text
                            logs.logs(
                                "INFO",
                                "Translate_dialogue",
                                f"Translated:\t{original_dialogue} -> {translated_text}",
                            )
                            break
