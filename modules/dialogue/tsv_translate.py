...
def main(input: str, output: str, fromlang: str, tolang: str, target: List[str]):
    ...
    with open(input, "r", encoding="utf-8") as f:
        ...
        for line in lines:
            ...
            match_str = re.match(r'^([^"]*)"((?:[^"\\]|\\.)*)"', contents) # エスケープされていないダブルクォーテーションのみを対象にする
            if match_str:
                ...
                dialogue = base16.encode_matches(r'\[.*?\]', dialogue)
                dialogue = base16.encode_matches(r'\\.', dialogue) # \{encoded_text} の形式だと変換されてしまうと思うので|encoded_text|のように変換
